#!/usr/bin/env python3
"""
Generate ALL Reference Audio Files for voices_2 experiment
...
# Voice configurations with NEW voice IDs
VOICES = {
    "v001": "68ad0ca7e68cb082a1c46fd6",  # male
    "v002": "68ad0cb625c2800730ac5b48"   # female
}

# Emotion configurations from plan.md
...
# Text content from docs/tts-test-sentences.md
EMOTION_TEXTS = {
    "angry": {
...
def create_reference_request(text, voice_id):
    """Create TTS request for reference (neutral) audio"""
    return {
        "text": text,
        "actor_id": voice_id,
...
def main():
    """Generate all reference audio files with HIGH QUALITY"""
    print("🎵 HIGH-QUALITY Reference Audio Generation for voices_2")
    print("=" * 60)
    
    for expressivity in ['expressivity_none', 'expressivity_0.6']:
        print(f"\n📁 Generating {expressivity} references...")
        
        output_dir = f"public/voices_2/{expressivity}/"
        ...
        for voice_name, voice_id in VOICES.items():
            ...
            for emotion in EMOTION_LABELS:
                for text_type in ["match", "neutral", "opposite"]:
                    ...
                    request = create_reference_request(text, voice_id)
            ...
            # Process emotion_vector emotions  
            for emotion, emotion_id in EMOTION_VECTORS.items():
                for text_type in ["match", "neutral", "opposite"]:
                    text_base = EMOTION_TEXTS[emotion][text_type]
        ...
        print(f"📊 Total {expressivity} references to generate: {len(all_requests)}")
    ...
    # Final verification
    total_files = len(os.listdir("public/voices_2/expressivity_none/")) + len(os.listdir("public/voices_2/expressivity_0.6/"))
    print(f"\n🎯 TOTAL REFERENCE FILES GENERATED: {total_files}")
    print(f"🎵 Expected: 144 reference files (72 per expressivity)")
    print("✅ HIGH-QUALITY Reference Generation Complete!")
