#!/usr/bin/env python3
"""
Complete Missing Sample Files for expressivity_0.6
...
def create_tts_request(text, voice_id, emotion_type, emotion_value, scale):
    ...
    request = {
        "text": text + "|0.6",  # Always add |0.6 suffix for expressivity_0.6
        "actor_id": voice_id,
...
def generate_missing_samples():
    """Generate only missing sample files for expressivity_0.6"""
    print("Generating MISSING SAMPLES for expressivity_0.6")
    print("=" * 60)
    
    output_dir = "public/voices_2/expressivity_0.6/"
    missing_files = []
    
    # Find missing emotion_label samples
    ...
    # Find missing emotion_vector samples
    for voice_name, voice_id in VOICES.items():
    ...
    print(f"Found {len(missing_files)} missing files to generate")
    
    if not missing_files:
        print("No missing files found! Generation complete.")
        return
    ...
    for batch_idx in range(0, len(missing_files), batch_size):
        batch = missing_files[batch_idx:batch_idx + batch_size]
        batch_requests = [item["request"] for item in batch]
        
        print(f"\nProcessing batch {batch_idx//batch_size + 1}/{(len(missing_files) + batch_size - 1)//batch_size}")
    ...
    print(f"\nCompleted: {success_count}/{len(missing_files)} files generated successfully")
