#!/usr/bin/env python3
"""
Generate MISSING Sample Audio Files for voices_3 - v4
Continue from where the previous generation left off due to token expiry
"""
...
# NEW FRESH API token from user
API_TOKEN = "Bearer eyJhbGciOiJSUzI1NiIsImtpZCI6ImUzZWU3ZTAyOGUzODg1YTM0NWNlMDcwNTVmODQ2ODYyMjU1YTcwNDYiLCJ0eXAiOiJKV1QifQ..NQ5Ea0yWL4M6TmkIQSxpS2WS4aUFjseDdbVtIUpufthjr-AKK7SQOen1ZQmDnZCTfDEofO8nqFiViTzMl1l8Qs9Zre01Dw2UrSJPvqHexCGT7TOiUXGlYB1cJew8jC_Zun98d9LNbtZFwO6b4GCNdjCWyKNcc4NCRxuwyT_prCeA4r3PI9suuzgsh3BNqJRfEO60N5T0HNpPbUbMhCnTB2q69uzV9UBx62z1ZBkRj2I6kh4sxRLXxoDJC5HYjX69sYpGOKk-cLc0WPc0hqbaWDGmTqDqQWGjMZ8FeZ4fWwCDC34zx8ZdS_oYmSwlBw0h4oeXaJziYbpfHkSm0KtS9w"
...
OUTPUT_DIR = "public/voices_3/expressivity_0.6/"
...
def get_missing_files():
    """Generate list of all expected files and check which ones are missing"""
    expected_files = []
    missing_requests = []
    missing_mappings = []
    ...
    return expected_files, missing_requests, missing_mappings

def main():
    """Generate only the missing sample files"""
    print("🎵 HD1-QUALITY Missing Sample Generation for voices_3")
    print("📋 Continuing from where we left off due to token expiry")
    print("=" * 60)
    
    expected_files, missing_requests, missing_mappings = get_missing_files()
    
    existing_count = len(expected_files) - len(missing_requests)
    print(f"📊 Total expected sample files: {len(expected_files)}")
    print(f"✅ Already generated: {existing_count}")
    print(f"🔄 Missing files to generate: {len(missing_requests)}")
    
    if len(missing_requests) == 0:
        print("🎯 All files already exist! No missing files to generate.")
        return
    ...
    for i in range(0, len(missing_requests), 4):
        batch = missing_requests[i:i+4]
        batch_mappings = missing_mappings[i:i+4]
        
        print(f"\n🔄 Processing batch {i//4 + 1}/{(len(missing_requests) + 3)//4} ({len(batch)} files)...")
    ...
    print(f"\n📈 Missing Files Generation Results:")
    print(f"✅ Success: {success_count}/{len(missing_requests)}")
    print(f"🎵 Quality Distribution: {quality_stats}")
    
    # Final verification
    total_files = len([f for f in os.listdir(OUTPUT_DIR) if f.endswith('.wav') and not f.endswith('_reference.wav')])
    print(f"\n🎯 TOTAL SAMPLE FILES NOW: {total_files}")
    print(f"🎵 Expected: 432 sample files")
    
    if total_files >= 432:
        print("✅ ALL SAMPLE FILES GENERATION COMPLETE!")
    else:
        print(f"⚠️  Still missing {432 - total_files} files")
